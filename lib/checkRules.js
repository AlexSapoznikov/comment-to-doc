...
import { ErrorHandling, TemplateStrictness } from './types';
export function validate(doc, config) {
    ...
    if (isError) {
        ...
        if ((errorHandling !== null && errorHandling !== void 0 ? errorHandling : ErrorHandling.Error) === ErrorHandling.Error) {
            throw new RuleError('Documents not generated due to the validation errors. For more info, see warnings above.');
        }
    }
    return isError;
}
export function checkRules(docData, rules) {
    var _a;
    ...
    return _a = {},
        _a[TemplateStrictness.Strict] = !isEqual(mandatoryRulesOrder, mandatoryDocTagsOrder),
        _a[TemplateStrictness.IgnoreOrder] = !isEqual(__spreadArrays((mandatoryRulesOrder || [])).sort(), __spreadArrays((mandatoryDocTagsOrder || [])).sort()),
